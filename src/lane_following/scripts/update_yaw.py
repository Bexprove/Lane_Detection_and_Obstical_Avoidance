...
class LaneFollower:
    ...
    def calculate_yaw_command(self, center_points, frame_width, left_line, right_line):
        if center_points:
            ...
            aggressive_offset = normalized_offset * 4.0

            max_yaw_rate_rad = 1.2
    ...
    def check_lane_gradient(self, frame):
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        grad_x = cv2.Sobel(gray, cv2.CV_64F, 1, 0, ksize=5)
        grad_y = cv2.Sobel(gray, cv2.CV_64F, 0, 1, ksize=5)
        magnitude = np.sqrt(grad_x**2 + grad_y**2)
        mean_magnitude = np.mean(magnitude)
        return mean_magnitude > 20  # Threshold on gradient strength
    ...
    def image_callback(self, msg):
        ...
        if not self.check_lane_gradient(cropped):
            yaw_command = 0.0  # Suppress turning if gradient is too weak

        twist = Twist()
        twist.linear.x = self.linear * (1 - min(abs(yaw_command), 1.0))  # reduce speed when turning
